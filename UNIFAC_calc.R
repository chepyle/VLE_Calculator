...
# install.packages(c('signal','minpack.lm','openxlsx'),lib='Rlibs')

library(openxlsx,lib='Rlibs')
library(minpack.lm,lib='Rlibs')
library(signal,lib='Rlibs')
...
Txy <- function(P,molecules){
  ...
  x1range<-c(10^seq(-5,-0.35,by=0.1),1-10^seq(-0.3,-5,by=-0.1))
  ...
  Tmin <- min(TPvap(P=P,mol=molecules))
  ...
  for (ix in seq(x1range)){
    ...
    fitT[ix] <-nls.lm(fitT0,lower=Tmin-20,upper=Tmax+5,
                      function(t){P-sum(partialP(v,x,t))})$par  # add +5/-20 range to pure BPs
    fitT0<-fitT[ix]
...
#  plot(x1range,x1range*exp(log_gam[,1]))
  ...
  df<-data.frame(x1=x1range,y1=y1range,x2=1-x1range,y2=1-y1range,Temperature=fitT)
  #setnames(df,c(paste0(molecules[1],"_Vap"),paste0(molecules[1],"_Liq"),paste0(molecules[2],"_Vap"),paste0(molecules[1],"_Liq"),'Temperature'))
return(df)
}
# 
# ## Test cases to run:
...
# 
# ## txy plot:
# txy<-melt(Txy(P,molecules),id.vars=c('Temperature','x2','y2'))
