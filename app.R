#App to run UNIFAC VLE calculation
#
   ...
   sidebarLayout(
      ...
      mainPanel(
        tabsetPanel(type = "tabs", 
                    ...
                    tabPanel("Txy_interactive", rbokehOutput("txyBokeh",height=800))
        )
      )
   )
))
  ...
  vle_data<-reactive({
   ...
   output$txyBokeh<-renderRbokeh({
     ...
     txy<-signif(vle_data(),digits=4)
      figure(tools=tools,
            title = paste0("Txy Diagram of ",input$mol1," & ", input$mol2),
            xlab=paste0(input$mol1, ' fraction'),
            width = 480,
            height= 640) %>%
